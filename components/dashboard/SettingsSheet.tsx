...
function SectionHeader({ title, onReset }: { title: string; onReset: () => void }) {
  return (
    <div className="flex items-center justify-between mb-3">
      <span className="text-[9px] uppercase tracking-[0.18em] text-zinc-500 font-semibold">{title}</span>
      <button
        onClick={onReset}
      ...
      <div className="flex justify-between text-[9px] text-zinc-400">
        ...
        <span className="flex gap-3">
          <span>
            ...
            warn {defaults.amber}{unit}
          </span>
          <span>
            <span className={cn("mr-0.5", redCls.text)}>▲</span>
            crit {defaults.red}{unit}
          </span>
        </span>
        <span>{max}{unit}</span>
      </div>
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-3">
        <div>
          <Label className="text-[10px] text-zinc-500 uppercase tracking-widest mb-2 block">Warning level</Label>
          <div className="flex gap-2">
            {amberOptions.map((c) => {
              ...
              return (
                ...
                >
                  <span className={cn("inline-block size-2 rounded-full", cls.dot)} />
                  {c}
                </button>
              )
            })}
        ...
        <div>
          <Label className="text-[10px] text-zinc-500 uppercase tracking-widest mb-2 block">Critical level</Label>
          <div className="flex gap-2">
            {redOptions.map((c) => {
        ...
        {/* Preview */}
        <div className={cn("flex items-center gap-2 px-3 py-2 border text-[10px] font-mono", AMBER_CLASSES[colors.amber].bg, AMBER_CLASSES[colors.amber].border)}>
          <span className={cn("size-2 rounded-full", AMBER_CLASSES[colors.amber].dot)} />
          <span className={AMBER_CLASSES[colors.amber].text}>DEGRADED — CPU high: 75%</span>
        </div>
        <div className={cn("flex items-center gap-2 px-3 py-2 border text-[10px] font-mono", RED_CLASSES[colors.red].bg, RED_CLASSES[colors.red].border)}>
          <span className={cn("size-2 rounded-full", RED_CLASSES[colors.red].dot)} />
          <span className={RED_CLASSES[colors.red].text}>CRITICAL — Memory critical: 97%</span>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <Sheet>
      <SheetTrigger asChild>
        ...
        >
          <Settings size={14} />
          <span className="hidden sm:inline uppercase tracking-widest">Config</span>
        </button>
      </SheetTrigger>
      ...
      >
        ...
        <Tabs defaultValue="polling" className="flex flex-col flex-1 min-h-0">
          <TabsList className="shrink-0 px-6 rounded-none bg-transparent border-b border-zinc-800 justify-start gap-4 h-10">
            <TabsTrigger value="polling"  className="text-[10px] uppercase tracking-widest pb-2.5 data-[state=active]:text-green-400">Polling</TabsTrigger>
            <TabsTrigger value="alerts"   className="text-[10px] uppercase tracking-widest pb-2.5 data-[state=active]:text-green-400">Alerts</TabsTrigger>
            <TabsTrigger value="colors"   className="text-[10px] uppercase tracking-widest pb-2.5 data-[state=active]:text-green-400">Colors</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="polling" className="flex-1 min-h-0 mt-0">
            <ScrollArea className="h-full">
              <div className="px-6 py-5 flex flex-col gap-6">
                <SectionHeader title="Refresh intervals" onReset={resetPolling} />

                <PollingRow label="/api/health"      endpoint="health"      defaultMs={POLLING_DEFAULTS.health} />
                <Separator className="bg-zinc-800" />
                <PollingRow label="/api/stats"       endpoint="stats"       defaultMs={POLLING_DEFAULTS.stats} />
                <Separator className="bg-zinc-800" />
                <PollingRow label="/api/metrics"     endpoint="metrics"     defaultMs={POLLING_DEFAULTS.metrics} />
                <Separator className="bg-zinc-800" />
                <PollingRow label="/api/collections" endpoint="collections" defaultMs={POLLING_DEFAULTS.collections} />
                <Separator className="bg-zinc-800" />
          ...
          <TabsContent value="alerts" className="flex-1 min-h-0 mt-0">
            <ScrollArea className="h-full">
              <div className="px-6 py-5 flex flex-col gap-6">
                <SectionHeader title="Alert thresholds" onReset={resetThresholds} />

                <ThresholdRow label="CPU usage"         metric="cpu"             unit="%" min={0}   max={100}  step={1} />
                <Separator className="bg-zinc-800" />
                <ThresholdRow label="Memory usage"      metric="memory"          unit="%" min={0}   max={100}  step={1} />
                <Separator className="bg-zinc-800" />
                <ThresholdRow label="Search latency"    metric="latency"         unit="ms" min={0}  max={2000} step={10} />
                <Separator className="bg-zinc-800" />
                <ThresholdRow label="Disk usage"        metric="disk"            unit="%" min={0}   max={100}  step={1} />
                <Separator className="bg-zinc-800" />
                <ThresholdRow label="Endpoint latency"  metric="endpointLatency" unit="ms" min={100} max={30000} step={100} />
                <Separator className="bg-zinc-800" />
                <SingleThresholdRow />
