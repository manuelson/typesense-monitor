...
export function HealthBanner({ level, alerts, loading, noData, onDismiss, onClearAll }: HealthBannerProps) {
  ...
  if (noData) {
    return (
      <div className="flex items-center gap-3 px-6 py-3 bg-amber-950 border-b border-amber-900">
        ...
        <span className="text-[11px] font-bold tracking-widest uppercase text-amber-400">SYSTEMS DISCONNECTED</span>
        <span className="text-amber-900 shrink-0">|</span>
        <span className="text-[10px] text-amber-700 tracking-wide">
          Check <code className="font-mono text-amber-600">TYPESENSE_HOST</code> and{" "}
  ...
  const bgColor = level === "red"   ? cn(redCls.bg,   redCls.border)
               : level === "amber" ? cn(amberCls.bg, amberCls.border)
               : "bg-green-950 border-green-900"

  const dotColor = level === "red"   ? redCls.dot
                 : level === "amber" ? amberCls.dot
                 : "bg-green-400"

  const textColor = level === "red"   ? redCls.text
                  : level === "amber" ? amberCls.text
                  : "text-green-400"
  ...
  return (
    <div className={cn("flex items-center gap-4 px-6 py-3 border-b", bgColor)}>
      <div className="flex items-center gap-2 shrink-0">
        <span className={cn("inline-block w-2 h-2 rounded-full", dotColor, level === "green" && "animate-pulse")} />
        <span className={cn("text-[11px] font-bold tracking-widest uppercase", textColor)}>
          {statusText}
        </span>
      </div>
