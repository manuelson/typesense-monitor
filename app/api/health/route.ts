...
export async function GET(request: Request) {
  ...
  if (host && apiKey) {
    try {
      const data = await createClientFromConfig(host, apiKey).health.retrieve()
      return Response.json(data)
    } catch {
      return Response.json({ ok: false, error: "unreachable" }, { status: 503 })
    }
  }
  ...
  try {
    const data = await getTypesenseClient().health.retrieve()
    return Response.json(data)
  } catch {
    return Response.json({ ok: false, error: "unreachable" }, { status: 503 })
  }
